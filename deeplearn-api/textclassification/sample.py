...
def read_data_sets(train_dir):
    ...
    for i in range(s_count):
        ...
        for j,_ in enumerate(s1_words):
            if s1_words[j] in embedding_dic:
                s1_vec.append(embedding_dic[s1_words[j]])
            else:
                s1_vec.append(embedding_dic['unknown'])
        for k,_ in enumerate(s2_words):
            if s2_words[k] in embedding_dic:
                s2_vec.append(embedding_dic[s2_words[k]])
            else:
                s2_vec.append(embedding_dic['unknown'])
    ...
    # 求得训练集与测试集的tensor并返回
    s1_image = np.array(s1_image,dtype='int64')
    s2_image = np.array(s2_image,dtype='int64')
